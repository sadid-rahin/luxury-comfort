...
export default function HostPanel() {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      // ✅ SECURITY FIX: Proxy Data via Backend URL
      const res = await fetch(`${BACKEND_URL}/sync-google`, { cache: 'no-store' });
  ...
  const handleDeleteBooking = async (e, booking) => {
    ...
    try {
      // ✅ SECURITY FIX: Proxy Request via Backend URL
        ...
        <div className="bg-white p-10 rounded-[3rem] text-slate-900 shadow-2xl sticky top-10 h-fit">
          ...
          <form onSubmit={handleDispatch} className="space-y-4">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
              <label className="text-[10px] font-black uppercase text-slate-400 block mb-1 text-left">Guest Details</label>
              <input value={selectedBooking ? getVal(selectedBooking, ['Guest_name', 'Guest Name', 'guest_name']) : ''} readOnly className="w-full bg-transparent font-bold text-sm outline-none mb-1 text-left" placeholder="Select a guest..." />
              <input value={selectedBooking ? getVal(selectedBooking, ['Guest_number', 'Guest Number', 'guest_number']) : ''} readOnly className="w-full bg-transparent text-xs text-slate-500 outline-none text-left" placeholder="Phone number..." />
            </div>
